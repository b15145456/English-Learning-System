...
function judge($str,$key){
		...
		if(strlen($str)-strlen($topic) > 2 || strlen($str)-strlen($topic) < -2){
			...
			$_SESSION["all_error"][] = $error_code_split[$i];
			}
		   
			if($_SESSION["quest_class"]>($_SESSION["number_select"]-1)){   
				header("Location: result_classical.php");   
				exit;
			}
			else{
			$rand_num=$_SESSION["classic"];
			$quest=$_SESSION["quest_class"];
			$classic_topic=$rand_num[$quest];
			$classic_voc=$rand_num[$quest][0];
			$_SESSION["all_voc_classic"][$quest]=$classic_voc;
			$quest++;
			$_SESSION["voc_classic"]=$classic_voc;
			$_SESSION["quest_class"]=$quest;
			$_SESSION["topic_classic"]=$classic_topic;
