...
function judge($str,$key){
		...
		if(strlen($str)-strlen($topic) > 2 || strlen($str)-strlen($topic) < -2){
			...
			if($_SESSION['quest_ocr']>($_SESSION["ocr_num"]-1)){   //####
				header("Location: result_ocr.php");   //####
				exit;
			}
			else{
			$rand_num=$_SESSION["resultArr"];
			$quest=$_SESSION['quest_ocr'];
			$topic=$rand_num[$quest];
			$voc=$rand_num[$quest][0];
			$_SESSION["all_voc_ocr"][$quest]=$voc;
			$quest++;
			$_SESSION["ocr_voc"]=$voc;
			$_SESSION['quest_ocr']=$quest;
			$_SESSION["ocr_topic"]=$topic;
